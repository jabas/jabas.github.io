...
///
	...
	var isProspectRestyle = $('#prospectLocMobileClose').length > 0;
	...
	body.on('click', '[data-toggle="headernav"]', function(e){
		...
		var visCollapse = $('.header-collapse.in')
		...
		if (mobileOpen) {
			...
			if (visCollapse.length > 0 && !isProspectRestyle) {
				$('.header-collapse.in').collapse('hide');	
			}
		} else {
			deactivateFocusLoop();
			if (visCollapse.length > 0 && isProspectRestyle) {
				visCollapse.removeClass('header-nested-in').addClass('header-nested-out-all');
				visCollapse.one('transitionend MSTransitionEnd webkitTransitionEnd', function() {
					visCollapse.collapse('hide');
					visCollapse.removeClass('header-nested-out-all');
				});
	...
	body.on('click', '#globalNavToggle, #globalNavToggle ~ .modal-backdrop', function(){
		var toggle = $('#globalNavToggle');

		if (toggle.hasClass('navbar-toggler-open')) {
			var modalBg = $('.modal-backdrop');
			toggle.removeClass('navbar-toggler-open');
			modalBg.removeClass('in');
			modalBg.one('transitionend MSTransitionEnd webkitTransitionEnd', function(){
				modalBg.remove();
			});
		} else {
			var modalBg = $('<div class="modal-backdrop fade"></div>');
			toggle.addClass('navbar-toggler-open');
			toggle.parent().append(modalBg);
	...
	body.on('keydown', function(e){
		if (e.which === 27) {
			...
			if ($('#globalNavToggle').hasClass('navbar-toggler-open')) { // to do - refactor DRY
				var modalBg = $('.modal-backdrop');
				$('#globalNavToggle').removeClass('navbar-toggler-open');
				modalBg.removeClass('in');
				modalBg.one('transitionend MSTransitionEnd webkitTransitionEnd', function(){
					modalBg.remove();
				});
	...
	body.on('show.bs.collapse', '.header-collapse', function(e){
		...
		if (mobileOpen) {
			if (isProspectRestyle) {
				$(this).addClass('header-nested-in');
			} else {
	...
	body.on('hide.bs.collapse', '.header-collapse', function(e){
		...
		if (isProspectRestyle) {
			$(this).removeClass('header-nested-in');
		}
